...
@section('breadcrumb')<span class="text-gray-800 font-medium text-sm">Pengelolaan Mata Kuliah</span>@endsection
...
@section('sidebar-nav')
    <a href="{{ route('student.courses.index') }}"
       class="nav-item {{ request()->routeIs('student.courses.index') ? 'active' : '' }}">
        ...
        <span x-show="sidebarOpen" x-cloak>Mata Kuliah</span>
    </a>

    {{-- Mata Kuliah accordion --}}
    <div x-data="{ open: true }">
        <button @click="open=!open" class="nav-item w-full text-left active">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                <rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/>
                <rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/>
            </svg>
            <span x-show="sidebarOpen" x-cloak class="flex-1">Mata Kuliah</span>
            <svg x-show="sidebarOpen" x-cloak class="w-3.5 h-3.5 transition-transform flex-shrink-0" :class="open?'rotate-180':''"
            ...
            <div class="h-32 bg-gradient-to-br from-blue-400 to-blue-600 relative overflow-hidden">
                ...
                <div class="absolute top-2 left-2">
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-white/90 backdrop-blur text-[10px] font-semibold text-blue-700 rounded-full">
                        @if($course->category->name === 'Projek')
                            ✕
                        @elseif($course->category->name === 'Teori')
                            ◎
                        @else
                            ◎
                        @endif
