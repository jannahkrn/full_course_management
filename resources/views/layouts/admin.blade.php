                ...
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" class="flex items-center gap-3 py-1.5 px-1 rounded-lg hover:bg-gray-50 transition-colors">
                        <div class="w-9 h-9 bg-gray-200 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden border border-gray-300">
                            ...
                            @else
                                <svg class="w-full h-full text-gray-400 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:4px_4px]" fill="none"></svg>
                            @endif
                        </div>

                        <div class="text-left hidden sm:block">
                            <div class="text-[14px] font-medium text-gray-800 leading-tight">
                                {{ auth()->user()->name ?? 'User Name' }}
                            </div>
                            <div class="mt-0.5">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-normal border bg-gray-50 text-gray-500 border-gray-200">
                                    Admin
                                </span>
                            </div>
                        </div>

                        <svg class="w-4 h-4 text-gray-500 flex-shrink-0 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="flex flex-1 overflow-hidden w-full">
        ...
        <aside class="flex-shrink-0 bg-white border-r border-gray-200 flex flex-col z-30 transition-all duration-200"
            ...
            <nav class="flex-1 overflow-y-auto overflow-x-hidden py-2"
                ...
                <div x-show="sidebarOpen" x-cloak class="nav-section-label" style="padding-top:8px;">Pembelajaran</div>
                <div x-show="!sidebarOpen" class="w-full border-t border-gray-100 my-2"></div>
                ...
                <a href="{{ route('admin.courses.index') }}"
                   ...
                   title="Mata Kuliah Saya">
                    ...
                    <span x-show="sidebarOpen" x-cloak class="flex-1">Mata Kuliah Saya</span>
                </a>

                <div x-data="{ open: false }" class="w-full">
                    <button @click="sidebarOpen ? open = !open : sidebarOpen = true" class="nav-item" :class="sidebarOpen ? '' : 'justify-center'" title="Aktivitas Pembelajaran">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/>
                        </svg>
                        <span x-show="sidebarOpen" x-cloak class="flex-1">Aktivitas Pembelajaran</span>
                        <svg x-show="sidebarOpen" x-cloak class="w-3.5 h-3.5 flex-shrink-0 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                    <div x-show="open && sidebarOpen" x-cloak class="space-y-0.5">
                        <a href="#" class="nav-sub-item">Semua Aktivitas</a>
                    </div>
                </div>

                <div x-data="{ open: false }" class="w-full">
                    <button @click="sidebarOpen ? open = !open : sidebarOpen = true" class="nav-item" :class="sidebarOpen ? '' : 'justify-center'" title="Evaluasi">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <circle cx="12" cy="12" r="9"/><path d="M9 12l2 2 4-4"/>
                        </svg>
                        <span x-show="sidebarOpen" x-cloak class="flex-1">Evaluasi</span>
                        <svg x-show="sidebarOpen" x-cloak class="w-3.5 h-3.5 flex-shrink-0 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                ...
                <div x-data="{ open: false }" class="w-full">
                    <button @click="sidebarOpen ? open = !open : sidebarOpen = true" class="nav-item" :class="sidebarOpen ? '' : 'justify-center'" title="Laporan">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/>
                            <line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>
                        </svg>
                        <span x-show="sidebarOpen" x-cloak class="flex-1">Laporan</span>
                        <svg x-show="sidebarOpen" x-cloak class="w-3.5 h-3.5 flex-shrink-0 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                ...
                <div x-show="sidebarOpen" x-cloak class="nav-section-label" style="padding-top:4px;">Administrasi Platform</div>

                <div x-data="{ open: {{ request()->routeIs('admin.users.*') ? 'true' : 'false' }} }" class="w-full">
                    <button @click="sidebarOpen ? open = !open : sidebarOpen = true" class="nav-item {{ request()->routeIs('admin.users.*') ? 'active' : '' }}" :class="sidebarOpen ? '' : 'justify-center'" title="Manajemen Pengguna">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4.03 3-9 3S3 13.66 3 12"/>
                            <path d="M3 5v14c0 1.66 4.03 3 9 3s9-1.34 9-3V5"/>
                        </svg>
                        <span x-show="sidebarOpen" x-cloak class="flex-1">Manajemen Pengguna</span>
                        <svg x-show="sidebarOpen" x-cloak class="w-3.5 h-3.5 flex-shrink-0 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                ...
                <div x-data="{ open: {{ request()->routeIs('admin.courses.*') || request()->routeIs('admin.categories.*') ? 'true' : 'false' }} }" class="w-full">
                    <button @click="sidebarOpen ? open = !open : sidebarOpen = true" class="nav-item {{ request()->routeIs('admin.courses.*') || request()->routeIs('admin.categories.*') ? 'active' : '' }}" :class="sidebarOpen ? '' : 'justify-center'" title="Manajemen Mata Kuliah">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/>
                        </svg>
                ...
                <div x-data="{ open: false }" class="w-full">
                    <button @click="sidebarOpen ? open = !open : sidebarOpen = true" class="nav-item" :class="sidebarOpen ? '' : 'justify-center'" title="Pengaturan Platform">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <circle cx="12" cy="12" r="3"/>
                            <path d="M19.07 4.93a10 10 0 010 14.14M4.93 4.93a10 10 0 000 14.14"/>
                        </svg>
                        <span x-show="sidebarOpen" x-cloak class="flex-1">Pengaturan Platform</span>
                        <svg x-show="sidebarOpen" x-cloak class="w-3.5 h-3.5 flex-shrink-0 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                ...
                <div x-data="{ open: false }" class="w-full">
                    <button @click="sidebarOpen ? open = !open : sidebarOpen = true" class="nav-item" :class="sidebarOpen ? '' : 'justify-center'" title="Sistem">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/>
                        </svg>
                        <span x-show="sidebarOpen" x-cloak class="flex-1">Sistem</span>
                        <svg x-show="sidebarOpen" x-cloak class="w-3.5 h-3.5 flex-shrink-0 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                ...
                <div x-data="{ open: false }" class="w-full">
                    <button @click="sidebarOpen ? open = !open : sidebarOpen = true" class="nav-item" :class="sidebarOpen ? '' : 'justify-center'" title="Keamanan & Privasi">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/>
                        </svg>
                        <span x-show="sidebarOpen" x-cloak class="flex-1">Keamanan & Privasi</span>
                        <svg x-show="sidebarOpen" x-cloak class="w-3.5 h-3.5 flex-shrink-0 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
