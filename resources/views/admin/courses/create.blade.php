...
@section('breadcrumb')
    ...
    <span class="text-gray-900 font-medium">Tambah Mata Kuliah</span>
@endsection
...
<div class="p-6">
    <form action="{{ route('admin.courses.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-xl font-bold text-gray-900">Tambah Mata Kuliah</h1>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <div class="grid grid-cols-1 gap-5">
                    ...
                    <div>
                        ...
                        <select name="category_id" class="select-field @error('category_id') border-red-400 @enderror">
                    ...
                    <div x-data="{
                        ...
                        get selectedTeachers() { return this.teachers.filter(t => this.selected.includes(String(t.id)) || this.selected.includes(t.id)) },
                        toggle(id) {
                            const sid = String(id);
                        ...
                        @error('teacher_ids')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="department_url" value="{{ old('department_url') }}"
                               placeholder="Masukkan url departemen"
                               class="input-field">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="template_course_id" class="select-field">
                            ...
                            @foreach($templateCourses as $tc)
                                <option value="{{ $tc->id }}" {{ old('template_course_id') == $tc->id ? 'selected' : '' }}>
                                    {{ $tc->title }}
                                </option>
                            @endforeach
                        </select>
                        <p class="mt-1 text-xs text-gray-400">Pilih mata kuliah sebagai templat untuk mata kuliah baru ini – isi dengan konten demo</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="access_type" class="select-field">
                            ...
                            <option value="restricted" {{ old('access_type') === 'restricted' ? 'selected' : '' }}>Restricted</option>
                        </select>
                    </div>
                        ...
                        <div class="flex gap-6">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="allow_unsubscribe" value="1"
                                       {{ old('allow_unsubscribe', '1') === '1' ? 'checked' : '' }}
                                       class="text-blue-600">
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="allow_unsubscribe" value="0"
                                       {{ old('allow_unsubscribe') === '0' ? 'checked' : '' }}
                                       class="text-blue-600">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="storage_limit_mb" value="{{ old('storage_limit_mb') }}"
                    ...
                    <div x-data="{
                        ...
                        <input type="text" x-model="input"
                               ...
                               placeholder="Mulai mengetik, lalu klik batang ini untuk memvalidasi tag"
                               class="input-field">
